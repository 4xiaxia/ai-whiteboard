...
- `RAILPACK_DEPLOY_APT_PACKAGES=chromium ffmpeg`，Railway/Railpack 运行时安装 Chromium 和 ffmpeg
...
- 内部流程：
  ...
  3. 后端启动 headless Chrome 打开本应用，并使用生产环境可访问的服务地址完成录制结果上传。
...
- 服务端会依次尝试 `CHROME_EXECUTABLE_PATH`、`GOOGLE_CHROME_BIN`、Linux 常见 Chromium 路径和本地 macOS Chrome 路径。
- Railway/生产环境需要同时具备 Chrome/Chromium 和 `ffmpeg`；Railway/Railpack 应设置 `RAILPACK_DEPLOY_APT_PACKAGES=chromium ffmpeg`，并可设置 `CHROME_EXECUTABLE_PATH=/usr/bin/chromium`。
